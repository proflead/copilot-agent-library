...
```bash
# Browser Automation with Interaction
copilot agent run brightdata-pro-agent "Go to https://news.ycombinator.com, snapshot the page, click the "More" link if visible, snapshot again, and list the top 5 story titles from the latest snapshot."

# Structured Extraction
copilot agent run brightdata-pro-agent "From https://github.com/brightdata/brightdata-mcp, extract the repo description, star count, and primary programming language as structured fields."
```

### IDE Usage

```
@brightdata-pro-agent Fetch structured metadata for this public YouTube video URL: <YOUTUBE_LINK> — title, channel, and view count if available. Cite the source URL
```
